...
LONG RegDeleteWinNTKey(HKEY basekey, LPCTSTR keynam)
{
	...
	{
		TCHAR subkeynam[MAX_PATH + 1];
		DWORD subkeylen = MAX_PATH + 1;
		for (DWORD i = 0; ; i++)
		{//Delete subkeys for WinNT
			subkeynam[0] = 0;
			res = RegEnumKey(tmp,i,subkeynam, subkeylen);
			if (res == ERROR_NO_MORE_ITEMS)
				break;
			DWORD numsub;
			res = RegQueryInfoKey(tmp, NULL, NULL, NULL, &numsub, NULL, NULL,
...
LONG RegCopyValues(HKEY src, LPCTSTR skey, HKEY dst, LPCTSTR dkey)
{
	...
	if (res == ERROR_SUCCESS)
		res = RegOpenKeyEx(src, skey, 0, KEY_READ, &sk);
	else
		RegCloseKey(dk);
	...
	if (res == ERROR_SUCCESS)
	{
		...
		TCHAR valbuf[MAX_PATH + 1];
		DWORD valbufsize;
		for (DWORD i = 0; ;i++)
		{
			valnamsize = valbufsize = MAX_PATH + 1;
			valbuf[0] = valnam[0] = 0;
			res = RegEnumValue(sk, i, valnam, &valnamsize, 0, &typ,
					(BYTE*) valbuf, &valbufsize);
